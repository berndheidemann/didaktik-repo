...
- **Sub-Pattern 2c: DOI/Volume/Pages stimmen, Titel und Topic gehören zu anderem Paper** (Welle-9-Erweiterung): Subtilere Hybrid-Variante als 2a/2b. Beispiel: Wu & Schunn (2023) Contemporary Educational Psychology, 73, 102160 — DOI führt zu real existierendem Wu&Schunn-Paper, aber dieses ist eine Peer-Feedback-Studie (n=367 AP-Schüler:innen), nicht der im Wiki referenzierte Classroom-Observation-Coding-Guide. Layer-1-Pflicht: zusätzlich zu Erstautor-Match auch **Titel-Match aus CrossRef-Response** gegen Wiki-Citation und Body-Behauptung abgleichen.
- **Pattern #4 Variante A: Body-Inline-Cite ohne Quellenliste-Eintrag** (Welle-5/Welle-10-Erweiterung): Body zitiert real existierende Quelle inline, aber Bibliographie am Artikelende enthält keinen passenden Eintrag. Formal MINOR (keine Halluzination), aber strukturelles Disziplin-Problem. Layer-3-Pflicht: Pro Body-Cite-Token (`Autor & Autor JJJJ` oder `Autor (JJJJ)`) prüfen, ob Erstautor in der `## Quellen`-Sektion auftaucht. Methodisch: Bulk-Sweep mit Skript ist effizienter als Per-Citation-Audit (siehe `2026-05-02-welle-10-bulk-sweep.md`).
...
2. **Hybrid-Citations (echter Titel + falsche Autoren)** (jetzt 17× repo-systematisch dokumentiert — stärkstes wiederkehrendes Pattern) — Lancaster/Cotton SIGITE 2023, Denny CACM 2024, Denny ITiCSE 2024, Jang KJME 2023, Brady L.M./Reeves/Garcia SciAdv 2020 (statt S.T. Brady/Cohen/Jarvis/Walton), Hannover/Zander 2020 (statt Kröske), Kotchian/McCall Programming Historian 2022 (statt Kirilloff 2021), Denny QWK-Misattribution Z. 148 (statt Pathak 2025), Marwan/Li 2025 SIGCSE, Tjärnhage/Zhang 2023 ECCE, de Oliveira Neto/Gonzalez-Barahona arXiv:2407.10322, Schalk/Schumacher npj Science of Learning 2023, Wu/Schunn CEP 2023 (Sub-Pattern 2c), Denny/Becker CACM 2024 (Welle 10) u. a. DOI-Lookup fängt diese NICHT, weil DOI fehlt oder zu echtem Paper mit anderen Autoren führt. Gegenmittel: Autorenliste UND Titel UND Venue jeweils einzeln gegen CrossRef prüfen.
   - **Sub-Pattern 2a (Welle 6, jetzt 4×): Erstautor-Initial-Verwechslung mit realer Namensvetterin/Namensvetter** — wenn der Nachname mehrere reale Forscher:innen im selben Feld umfasst, reicht DOI-Lookup nicht. Beispiele: S. T. Brady (Wake Forest) ↔ L. M. Brady (Univ. Washington) im Belonging-Feld; Lennart Schalk ↔ Ralph Schumacher (beide ETH-Zürich) in der ICAP-Kritik. Diskriminator: Co-Autorenliste muss jeweils matchen.
   ...
   - **Sub-Pattern 2c (Welle 9): DOI/Volume/Pages stimmen, Titel und Topic gehören zu anderem Paper** — Beispiel Wu & Schunn 2023 in CEP 73, 102160: DOI führt zu real existierendem Paper (Peer-Feedback-Studie n=367), aber Wiki referenziert Topic eines anderen Papers (Classroom-Observation-Coding). Layer-1-Pflicht: Titel-Match aus CrossRef-Response zwingend zusätzlich zu Erstautor-Match.
...
4. **Body-vs-Quellenliste-Inkonsistenz** (Welle 5+6+10) — drei Varianten:
   - Variante A (Welle 5+10, strukturell weit verbreitet): Body-Inline-Cite ohne Quellenliste-Eintrag. Quelle ist real und korrekt attribuiert, aber fehlt in der `## Quellen`-Sektion. Welle-10-Bulk-Sweep fand ~20 Treffer in 14+ Dateien, davon ein Drittel in bereits auditierten Artikeln (Welle 1-9 hat das Pattern nicht systematisch geprüft). Gegenmittel: Skript-basierter Body-Inline-Sweep als Standard-Layer-3-Check.
   ...
   - Variante C (Welle 5): Phantom-Co-Autor im Body, Quellenliste single-author korrekt (Beispiel Ohmann/Soosai Raj; Conlen/Conlen/Heer 2018 in W8 explorable-explanations).
   Gegenmittel: Body-Cite-Tokens (`Autor JJJJ`) gegen Quellenliste UND gegen Body-Aussage in Layer 3 abgleichen.
...
**Cross-Repo-Sweep-Ausbeute sinkt:** Sweep-1 (nach Welle 4) fand 6 MAJORs, Sweep-2 (nach Welle 5) 6 MAJORs — beide mit geteilten Citations-Vorlagen als Ursache. Sweep-3 (nach Welle 6) fand **0 Befunde** — Welle-6-Bugs sind idiosynkratisch (Einzelfall-Hybrid-Citations), nicht aus Vorlagen entstanden. Sweep 4 (W7) 0 Befunde, Sweep 5 (W8) 1 MAJOR (Margulieux d=0,44 in meta-analysen-cs-education), Sweep 6 (W9) 1 MINOR (Zingaro-Paraphrase, gleicher Artikel). **Repeat-Offender bestätigt:** `meta-analysen-cs-education.md` ist als Aggregat-Übersichts-Artikel der einzige systematische Treffer-Punkt — bei jedem Methoden-Fix proaktiv mitziehen. Konsequenz: Cross-Repo-Sweeps bleiben sinnvoll für Vorlagen-Pattern (Datierungen, Verwaltungs-Cites, Hattie-Edition-Drift) und für Aggregat-Artikel-Spillover; bei reinen Hybrid-/fabrizierten-Citations ist der Sweep meist clean — der Schutz muss in den Audits selbst liegen.

## Welle-10-Erkenntnis: Strukturelle Bulk-Sweeps als komplementäre Methodik

**Welle 10 hat die Pattern-Heuristik radikalisiert** und Score-Auswahl + Per-Citation-Subagents komplett ersetzt durch zwei strukturelle Sweeps:
- **Phase A — DOI/arXiv-Bulk-Sweep**: Skript extrahiert alle DOIs/arXiv-IDs aus nicht-auditierten Artikeln, Bulk-CrossRef-Lookup, Erstautor-Match. Ressourcen-effizient: ~30k Token statt ~280k pro lineare Welle.
- **Phase B — Body-Inline-vs-Quellenliste-Sweep**: Skript extrahiert Body-Inline-Cite-Tokens repo-weit, gleicht gegen Quellenliste-Sektion ab. Findet Pattern #4 Variante A strukturell — und als Beifang fabrizierte Studien-Boxen (Pattern #3+#5), wenn die generischen Cite-Erstautoren in CrossRef nicht reproduzierbar sind.

**Ergebnis Welle 10:** 4 CRITs (Becker→Denny CACM 2024 Hybrid, Leppink 2023 fabriziert, Tafliovich 2023 fabriziert, Yang 2024 fabriziert + 2× Vorlagen-Hallucination) + ~20 MINORs (Pattern #4 Variante A) — vergleichbar mit Welle 8 bei ~10 % des Token-Aufwands.

**Methodik-Empfehlung:** Strukturelle Bulk-Sweeps als **Standard-Folge-Welle** nach jeder Per-Citation-Welle einsetzen, nicht als Ersatz. Per-Citation-Audits finden Hot-Spot-Inversionen + Effektstärken-Drifts + Tabellen-Klassifikations-Fehler (Pattern #6, #7, #8, #10) zuverlässiger; Bulk-Sweeps fangen Hybrid-Citations (Pattern #2) + fabrizierte Cites (Pattern #5) + Bibliographie-Disziplin (Pattern #4 Variante A) effizienter.
